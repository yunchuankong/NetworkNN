...
with tf.Session() as sess:
    for t in range(int(trials)):
        ...
        for g in range(n_features):
            ...
            var_imp[t, g] = np.mean(abs(y_s_g[:, 1] - y_s[:, 1])/d_g) ## average feature importance over samples
    ...
    np.savetxt(str(n_instances) + "s_" + str(n_features) + "g_" + "varImpWithNetwork.csv",
