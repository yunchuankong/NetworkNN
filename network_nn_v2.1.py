...
with tf.Session() as sess:
    ...
    for epoch in range(training_epochs):
        ...
        if epoch % display_step == 0:
            ...
            acc, y_s = sess.run([accuracy, y_score], feed_dict={x: expression, y: labels, keep_prob: 1})
            auc = metrics.roc_auc_score(labels, y_s)
    ...
    ## Testing cycle
    for _ in range(int(testing_times)):
        ...
        avg_test_acc += acc / testing_times
        avg_test_auc += auc / testing_times
    print ("Average testing accuracy, ", avg_test_acc)
    print ("Average testing auc, ", avg_test_auc)
